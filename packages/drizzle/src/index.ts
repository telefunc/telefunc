...
// Drizzle query you terminate with `.live()` to get a `Live` instead of rows. Writes are not observed
// yet: `insert`/`update`/`delete` run as plain Drizzle, so a live query goes stale only when something
// invalidates it. The IR / extraction / read-capture engine internals are implementation detail and are
