...
// subscriber feeds the whole batch into its graphs via `router.ingest` and the router slices it per table.
// Each test INJECTS its own in-process transport (setChangeTransport) so subscribers never leak across
// tests; registryFor is mocked so router.ingest is observable.
//
...
let counter = 0
const remoteBatch = (changes: TableChange[]) => ({ origin: `remote-${counter++}`, changes })
...
describe('write transport — one topic, one publication per batch', () => {
  it('a MULTI-TABLE batch is published once and ingested once, whole', async () => {
    ...
    const wirePublications: unknown[] = []
    transport.subscribe(CHANGE_TOPIC, (message) => {
      if (!('probe' in message)) wirePublications.push(message)
    })
    transport.publish(CHANGE_TOPIC, remoteBatch(changes))

    expect(wirePublications).toHaveLength(1) // ONE message on the wire for a two-table commit
    expect(engine.ingest).toHaveBeenCalledTimes(1) // ...and one ingest
  ...
  it('TWO dbs sharing one transport: A publishes → B applies, A suppresses its own echo', async () => {
    // The multi-instance path in one process: distinct registries, one shared transport.
    const transport = createInMemoryChangeTransport()
    const dbA = {}
    const dbB = {}
...
describe('write transport — coarse-all rides the SAME topic', () => {
  ...
  it('a coarse-all with nothing watched ingests nothing', async () => {
    ...
    setChangeTransport(dbB, transport)
    await ensureSubscribed(dbA)
    await ensureSubscribed(dbB) // B has no registered graphs
    publishCoarseAll(dbA)
    expect(engine.ingest).not.toHaveBeenCalled()
  })
})
