...
import { type ExportList, getExportList, getTelefunctionRef } from '../getExportList.js'
...
function generateShieldCode({
  ...
  const processSide = (d: Discovered, sites: Site[], aliasSuffix: string, applyFn: string) => {
    ...
    return { code: entries ? `\n${applyFn}(${getTelefunctionRef(d.e)}, { ${entries} });` : '', failed: false }
  }
  ...
  for (const d of discovered) {
    ...
    if (d.mainShield !== undefined)
      shieldCode += `\n__telefunc_shield(${getTelefunctionRef(d.e)}, ${d.mainShield}, { __autoGenerated: true });`
