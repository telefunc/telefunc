...
//
// Server-side reactive integrations reach the PRODUCER end through the extension HOST
// (`TelefuncServerExtension.setup(host)` → `host.createLive`), NOT here — so this entry no longer exposes
// `LiveCell`, which was @telefunc/drizzle's only cross-package reach into telefunc internals.
