...
/** Normative transition data; non-TypeScript backends own their runtime rendering. */
...
function assertHeadTransition(
  ...
  const rule = HEAD_TRANSITIONS.find(
    (candidate) => candidate.from === from && candidate.cx === form && candidate.to === next.head.state,
  )
