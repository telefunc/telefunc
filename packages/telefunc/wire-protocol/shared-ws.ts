...
function decodePublishBinary(wire: Uint8Array): { data: Uint8Array; info: WirePublishInfo } {
  assert(wire.byteLength >= PUBLISH_BINARY_PREFIX.byteLength, 'PUBLISH_BINARY frame too short for version header')
  const view = new DataView(wire.buffer, wire.byteOffset, wire.byteLength)
  const versioned = view.getUint16(0, true) === 0x4654 && wire[2] === 0x42 && Number.isNaN(view.getFloat64(4, true))
  assert(versioned, 'PUBLISH_BINARY frame uses an unsupported legacy wire format')
