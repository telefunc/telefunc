...
import { convertMessagesToModelMessages, EventType } from '@tanstack/ai'
...
async function* callProvider(prompt: string, signal: AbortSignal): AsyncGenerator<StreamChunk> {
  ...
  yield { type: EventType.RUN_STARTED, runId, threadId, timestamp: now }
  ...
  yield { type: EventType.RUN_FINISHED, runId, threadId, finishReason: 'stop', timestamp: Date.now() }
}
