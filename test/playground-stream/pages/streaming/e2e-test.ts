...
function testStreaming() {
  ...
  test('streaming: Abort in one producer aborts all multiplexed streaming values', async () => {
    ...
    await autoRetry(async () => {
      ...
      expect(result.obsValues.length).greaterThan(0)
      expect(result.subjectValues.length).greaterThan(0)
      ...
      expect(result.obsErr?.isAbort).toBe(true)
      expect(result.subjectErr?.isAbort).toBe(true)
      const abortValue = { reason: 'stream-abort', code: 101 }
      expect(result.abortingErr?.abortValue).deep.equal(abortValue)
      expect(result.otherErr?.abortValue).deep.equal(abortValue)
      expect(result.streamErr?.abortValue).deep.equal(abortValue)
      expect(result.promiseErr?.abortValue).deep.equal(abortValue)
      expect(result.obsErr?.abortValue).deep.equal(abortValue)
      expect(result.subjectErr?.abortValue).deep.equal(abortValue)
    })
  })
