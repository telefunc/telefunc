...
| Text channel | One room per channel. **Viewing** = membership (grants publish), obtained via the `onOpenChannel` telefunction; only the *open* channel's text lane is subscribed — unread dots for the rest ride the body-free `onActivity` signal |
| Message history | `messages` table, written by the channel's `onPublish` guard (`author_id: from.identity`), replayed with the docs' subscribe → join → fetch recipe (but see finding 14), paginated with a `beforeAt` cursor |
...
| Voice/video | One room per voice channel; capacity enforced by its `onJoin` guard. Mic/camera/screen are **named binary tracks** (`publishBinary(frame, { track, keyFrame })` / `subscribeBinary(cb, { track })`); mute/camera/screen state rides `setMeta` |
...
what the round-2 migration deleted.
...
- Round 2 receipts: `userId` is gone from `MemberMeta` (`shared/types.ts`) — display state and
  identity are separate concerns now; the `onPublish` guard persists `author_id: from.identity`
  ...
  (`app/store.ts`). The trade: every join must go through a telefunction — which is the right
  security posture anyway, but it interacts badly with the history fence (finding 14).
...
(`telefunc/dms.telefunc.ts`); after that, the only job left for the `onSend` guard is to close
...
### 5. Every state write is a full replace — half adopted

- Adopted: `Room.update` patches per-field now — updating a topic no longer re-supplies `size`
  (a finite cap used to silently reset to `Infinity`). Receipt: `onSetTopic` in
  `telefunc/channels.telefunc.ts` sends `{ meta }` alone.
- Still open: `setMeta` replaces whole metadata — every toggle is a spread
  (`setMeta({ ...meta, muted })`, three times in `app/call.ts`; status in `app/store.ts`), and
  concurrent updates to *different* fields still clobber each other (LWW is per-record, not
  per-field). Suggestion: `setMeta(patch, { merge: true })`.
...
`openChannel`/`sendMessage` in `app/store.ts`) is subtle enough to deserve a docs recipe. Now
that joins are telefunctions (finding 1), the window is *wider* — a full HTTP round-trip.
...
### 10. Unread badges cost full subscription — ✔ adopted upstream

Discord-style unread indicators used to require subscribing to every channel's text lane — full
message traffic for channels nobody is reading.

- Adopted: `room.onActivity()` — a throttled, body-free control-lane signal.
- Round 2 receipts: only the open channel subscribes to messages (`openChannel` swaps one
  subscription); every other channel carries just the activity signal → unread dot
  (`wireChannel` in `app/store.ts`). It's deliberately not a count — Discord's plain-unread dot
  matches exactly.
...
- Adopted: an `onJoin` guard next to `onPublish`/`onSend`.
- Round 2 receipts: voice rooms are guarded `onJoin: () => { if (channel.isFull) throw … }`
...
- Still open: the `onPublish` guard has no timestamp/seq — history rows get `Date.now()` at
  persist time while live renders use `info.timestamp`; two clocks for one message.
...
Migrating onto the adopted APIs surfaced three new items.

### 14. Server-side joins break the lossless-history fence

The docs' lossless-history recipe (subscribe → join → fetch: "the join ack proves the
subscription is active, so nothing published after the fetch snapshot can be missed") assumes
the join rides the room's own connection. But trusted `identity` made **telefunctions the
blessed join path** (finding 1) — and a telefunction join travels the HTTP lane, so its ack
proves nothing about the WebSocket subscription. This app's fence is heuristic now: subscribe,
then two full round-trips (join + history fetch) and hope the subscription attached in between
(note in `openChannel`, `app/store.ts`). It has never dropped a message in practice — the
subscription attach wins the race by a wide margin — but "wide margin" isn't "fence".

- Suggestion: make the server-side join await the joiner's subscription state (it knows the
  member's channel), or expose a client-side barrier (`await room.subscribed()`), or an
  identity-token variant (`Room.mintJoinToken(identity)` → client joins over its own connection
  presenting the token) so identity and the connection-ordered ack can coexist.
...
|---|---|---|
| `Room.create` / `Room.get` / `Room.getOrCreate` / `Room.guard` / `Room.update` / `Room.close` | ✔ | `server/rooms.ts`, `server/guards.ts`, `telefunc/channels.telefunc.ts` |
| `Room.removeParticipant({ identity }, { reason })` / `Room.announce` / `Room.send` | ✔ | kick sweep, banners + directory feed, DM delivery |
