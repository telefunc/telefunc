...
> Telefunc guarantees that **every** value arriving at the server is validated — not just a telefunction's top-level arguments, but also every value the client sends through a <Link href="/streaming" text="streaming primitive" /> (callbacks, channels, streams).
