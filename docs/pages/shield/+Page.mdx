...
<Link href="/channel#new-broadcastchannel" text={<code>{'new BroadcastChannel<T>({ key })'}</code>} /> declares the message type `T` flowing through a keyed broadcast topic. Telefunc generates a `data` sub-shield that fires on the server when a client calls `publish()` on a `BroadcastChannel` returned from a telefunction:
...
export async function onJoinChat(roomId: string) {
  const room = new BroadcastChannel<{ user: string; text: string }>({ key: `chat:${roomId}` })
  return room
}
```
