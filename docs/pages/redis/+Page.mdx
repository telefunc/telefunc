...
Redis-backed <Link href="/channel#broadcast">broadcast</Link> fan-out across server instances — `publish()` on one server instance reaches `subscribe()` on all other server instances.
