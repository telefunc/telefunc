...
> At the telefunction boundary, Telefunc interprets an error your application throws as **unexpected** unless it is `Abort()`. Framework APIs can also reject with their own documented operational errors; Rooms are one example below.
...
### Room operational errors

<Link href="/room">Room</Link> operations can fail because the room closed, a participant left, or a target isn't a member. These are expected framework conditions, not bugs in your backend code: Telefunc preserves their safe message and doesn't trigger <Link href="/onBug"><code>onBug()</code></Link>.

Telefunc calls this internal category `RoomError`, but the class isn't exported. A direct server-side Room call rejects with an error whose `name` is `'RoomError'`; after it crosses the Room channel, the client receives an ordinary `Error` carrying the same safe `message`. Don't use `instanceof RoomError`. If your own policy needs a stable code or value, reject from a Room guard with `Abort({ code: ... })` instead.
