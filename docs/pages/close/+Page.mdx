...
`close()` and `onClose()` manage the lifecycle of the streams and connections a telefunction returns or accepts as arguments — so the server can stop producing and release resources.
...
| `onClose(cb)` | server | — (cleanup hook) | Fires when the value ends, however that happens |
...
All of these tell the server to stop producing and free its resources.
