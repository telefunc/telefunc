...
> By default, channels and broadcasts use **SSE** and work without extra server setup. When WebSocket is enabled (see <Link href="/Telefunc" />), the client starts on SSE and transparently upgrades to WebSocket in the background (see <Link href="/transport" />).
...
`new BroadcastChannel({ key })` extends broadcasting to the client-side (whereas `Broadcast` is server-side only).
