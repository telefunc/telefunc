...
`new BroadcastChannel({ key })` creates one member of a broadcast group. All instances created with the same `key`, on the server or on clients, form the group: `publish()` delivers a message to every subscriber of the key, including the publishing instance itself, and `subscribe()` receives every message published to the key. Return the `BroadcastChannel` instance directly from a telefunction — no `.client` needed (unlike channels, broadcast has no directional type flip).

The name reflects the shape: a `Channel` to the one client that received it, bridged onto a keyed broadcast group. That same group is reachable on the server without an instance, through the static `Broadcast.*` bus — see <Link text="Server-side Broadcast" href="#server-side-broadcast" />.
...
A `BroadcastChannel` bridges a *client* into a key's group. To publish or subscribe purely on the **server** — no client, no handle, no lifecycle — use the static `Broadcast.*` methods instead. They take the `key` as their first argument and reach the same group: `Broadcast.publish(key, …)` is delivered to every member of the key, including any client holding a `BroadcastChannel` for it.

```ts
import { Broadcast } from 'telefunc'

// Fire-and-forget publish to every subscriber of the key
...
> The instance has its `key` baked in, so `publish(data)` / `subscribe(cb)` take no key. The static <Link text={<code>Broadcast.publish(key, data)</code>} href="#server-side-broadcast" /> / `Broadcast.subscribe(key, cb)` take it first — and have no `onOpen` / `onClose` / `close` / `abort`, since there's no instance to manage.
