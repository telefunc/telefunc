...
The receipt additionally carries `receivers` — the key's live subscription count (in-memory subscribers, Redis `PUBLISH` receivers, Cloudflare DO presence). `0` truthfully means nobody anywhere is subscribed right now: the signal for a producer to pause expensive work until someone tunes in. It counts subscriptions at the transport hop — in-memory listeners, or peer instances subscribed over Redis/Cloudflare — not the end viewers those instances may each serve.
