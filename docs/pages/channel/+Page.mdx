...
> By default, channels and broadcasts use [SSE](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events) and work without extra server setup. When <Link href="/transport#channel-transport">WebSocket is enabled</Link>, the client starts on SSE and upgrades to WebSocket in the background.
...
In-memory and standalone Redis report an authoritative `receivers` count; `0` means no transport subscription was reachable when the publish was accepted. Cloudflare also reports `receivers`, but its cross-region presence registry is eventually consistent: the count can temporarily lag joins or leaves, so `0` isn't proof that nobody is subscribed. Redis Cluster omits the field because `PUBLISH` reports only a node-local count. The field always counts transport subscriptions, not the end viewers each peer may serve.
