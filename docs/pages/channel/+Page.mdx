...
> By default, channels and broadcasts use **SSE** and work without extra server setup. When WebSocket is enabled (see <Link href="/server" />), the client starts on SSE and upgrades to WebSocket in the background (see <Link href="/transport" />).
