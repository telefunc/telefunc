...
`seq` is monotonic per `key`, which is useful for ordering and gap detection. `publish()` resolves to `info` once the message is accepted. A publish right after a `subscribe()` on the same server reaches that subscriber: while the new subscription is being set up, the publish waits for it, for up to 60 seconds.
