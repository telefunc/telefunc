...
An instance `new Channel()` or `new BroadcastChannel()` is returned from a telefunction and passed to the client (it serializes into client-side objects automatically). `Broadcast` is a server-side API, so there's nothing to return.
