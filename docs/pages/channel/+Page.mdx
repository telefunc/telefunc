...
In-memory, standalone Redis, and Cloudflare report `receivers` because they can count globally; `0` truthfully means nobody anywhere is subscribed. Redis Cluster omits it because `PUBLISH` reports only a node-local count. It counts transport subscriptions, not the end viewers each peer may serve.
