...
`new BroadcastChannel({ key })` enables you to broadcast also on the client-side.

> It creates a `Channel` to the one client that received it, and bridges that client onto a `Broadcast` key.
>
> Return it from a telefunction and that client can `publish()` / `subscribe()` on the key — every message reaches every member of the group (both client- and server-side).
