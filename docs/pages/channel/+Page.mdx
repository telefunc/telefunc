...
By default, channels and broadcast use **SSE** and work without extra server setup. When WebSocket is enabled (see <Link href="/server" />), the client starts on SSE and upgrades to WebSocket in the background. See <Link href="/transport" />.
...
// Receive every message published to the key, including this instance's own publishes
