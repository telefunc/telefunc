...
With the Node.js [`req` object](https://nodejs.org/api/http.html#class-httpincomingmessage) and [`res` object](https://nodejs.org/api/http.html#class-httpserverresponse) (Express.js):
...
> On Cloudflare the `context` is set in a separate function from the request handler, because it runs in a Durable Object whereas the request handler doesn't.
...
> `telefunc.serve()` returns a falsy value (`undefined` or `false`) for non-Telefunc requests, allowing you to chain it with other handlers.
