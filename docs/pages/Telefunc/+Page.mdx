...
With the web standard [`Request` object](https://developer.mozilla.org/en-US/docs/Web/API/Request) and [`Response` object](https://developer.mozilla.org/en-US/docs/Web/API/Response) (Hono, Fastify, ...):
...
With the Node.js [`req` object](https://nodejs.org/api/http.html#class-httpincomingmessage) and [`res` object](https://nodejs.org/api/http.html#class-httpserverresponse) (e.g. [Express](https://expressjs.com)):
...
Add the AsyncLocalStorage flag, and the Durable Object and KV bindings, to your `wrangler.jsonc`:
...
> See <Link href="/stream/cloudflare" /> for scaling, distributed broadcast, delivery guarantees, and Durable Object configuration.
...
| `installWebSocket(server)` | Node | Enable WebSocket channels on your HTTP server. Idempotent (calling it multiple times is safe). (The Node.js adapter auto-detects your HTTP server from the request socket.)|
...
| Node.js (`{ req, res }`) | `boolean` | `false` if not a Telefunc request |
