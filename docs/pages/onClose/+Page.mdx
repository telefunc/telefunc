...
> **How does it work?** When a `stream` object becomes unreachable (the client drops all references to it), the browser's garbage collector clears `stream` and, when this happens, Telefunc automatically closes the stream.
