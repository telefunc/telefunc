...
When using `@telefunc/tanstack-query`, keys prefixed with `global:` invalidate globally: *every* connected client with a matching query refetches.

> All other keys remain normal (invalidate locally, i.e. current tab only).

> **How does it work?** Under the hood, the `global:` key is sent to the server and then <Link href="/channel#broadcast">broadcast</Link> to every client whose query key matches. Learn more at <Link href="#how-it-works" />.
