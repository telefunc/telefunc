...
The `@telefunc/tanstack-query` integration uses <Link href="/stream">Telefunc Stream</Link> under the hood (in particular <Link href="/channel">channels</Link> and <Link href="/channel#broadcast">broadcasts</Link>).

### Local keys

Queries: normal (`@telefunc/tanstack-query` has no effect).
...
1. If it's <Link href="#local-vs-global-keys">a global key</Link>, then <Link href="#install">`withTelefunc()`</Link> sends `queryKey` alongside the query telefuncion call.
...
1. Every connected client with a subscription that matches the gloabl key calls [`invalidateQueries()`](https://tanstack.com/query/latest/docs/framework/react/guides/query-keys).

Mutations:
1. <Link href="#install">`withTelefunc()`</Link> sends global keys from <Link href="#mutations">`meta.invalidates`</Link> alongside the telefuncion mutation call.
1. The server <Link href="/channel#broadcast">broadcasts</Link> an invalidation event to the clients that use a query key that matches a global key.
