...
> A key is global when its *first* element is a string starting with `global:` — e.g. ``['global:todos', `user:${userId}`]{:js}`` is global, while `['todos', 'global:x']{:js}` is local (its first element doesn't start with `global:`).

> Global keys are carried by Telefunc calls, so two rules apply:
>
> - **`queryFn` must call a telefunction.** With a non-telefunc `queryFn` (e.g. plain `fetch()`), a global key silently behaves like a local one. The same goes for mutations: global keys in `meta.invalidates` are published server-side after the telefunction succeeds — so `mutationFn` must be a telefunction call too.
