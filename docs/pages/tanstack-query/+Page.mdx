...
`withTelefunc()` returns the same `QueryClient` instance. All options and APIs continue to work as before.

> The integration lives in `QueryClient`, so any TanStack Query adapter works: React, Vue, Svelte, Solid. (The peer dependency is `@tanstack/query-core` v5+.) The examples above use React.
...
> A key is global when its *first* element is a string starting with `global:` — e.g. `['global:todos']` is global, while `['todos', 'global:x']` is local (its first element doesn't start with `global:`).
...
> Invalidation works across multiple servers via a <Link text="broadcast transport" href="/channel#multi-server" /> such as <Link href="/redis" text={<code>@telefunc/redis</code>} />.
