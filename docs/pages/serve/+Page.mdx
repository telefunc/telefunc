...
Low-level function that turns a telefunction HTTP request into an HTTP response. It's a pure function: stateless and side-effect-free. It runs in any runtime without an adapter.
...
>   > You can still use <Link href="/stream">Telefunc Stream</Link> but Telefunc won't use the WebSocket transport. (Another transport will be used.)
