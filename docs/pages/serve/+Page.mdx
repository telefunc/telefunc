...
The `serve()` function enables you to <Link href="/server">install a Telefunc middleware into any server</Link>.

> It's a pure function (no side effects).

> `serve()` was previously named `telefunc()`. The old name `telefunc()` still works but is deprecated and will be removed in the next major release.
...
> The `context` parameter is optional and is only needed if you use `getContext()`, see <Link href="/getContext#provide" />.
