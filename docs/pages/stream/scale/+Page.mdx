...
- **Cross-instance backend** (a <Link href="/channel#broadcast">broadcast</Link> or a <Link href="/room">room</Link> must reach subscribers on every server instance) — install one such as <Link href="/redis">`@telefunc/redis`</Link>.
  > Optional: this is required only for broadcasts and rooms.
...
In a single-instance setup, the default in-memory backend is enough. In a multi-instance setup, install a backend that fans out across the cluster — such as <Link href="/redis"><code>@telefunc/redis</code></Link> — or configure a broadcast transport.

> A broadcast transport (NATS, Kafka, RabbitMQ, …) provides cross-instance Broadcast only. <Link href="/room">Rooms</Link> require a shipped full backend such as <Link href="/redis"><code>@telefunc/redis</code></Link> or the Cloudflare adapter.

> For Cloudflare Workers, Telefunc has an adapter that uses Durable Objects — see <Link href="/stream/cloudflare" />.

### `BroadcastTransport`

Implement `BroadcastTransport` and assign it to `config.broadcast.transport`:
...
`send()` and `sendBinary()` return the assigned `{ seq, timestamp }`, giving every node one global order per key (shared by text and binary): `seq` is a positive safe integer and `timestamp` a non-negative safe integer such as `Date.now()`. `listen()` and `listenBinary()` return an unsubscribe function, and a key has at most one live listen of each kind: the next listen for it comes after that function ran. A `send()` on the same node after `listen()` returned must reach that listener: that is what delivers a publish made right after a `subscribe()`. A configured broadcast transport takes precedence over an installed backend's broadcast plane, which takes precedence over the in-memory default. Set it once, before the first Broadcast subscription: changing it while subscriptions are open throws, since they would stay on the replaced transport.
