...
`send()` and `sendBinary()` return the assigned `{ seq, timestamp }`, giving every node one global order per key; `listen()` and `listenBinary()` return an unsubscribe function. A configured broadcast transport takes precedence over an installed backend's broadcast plane, which takes precedence over the in-memory default.
