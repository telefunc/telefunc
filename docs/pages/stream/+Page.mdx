...
To save resources, you can cancel a stream at anytime by <Link href="/close">manually closing it</Link> — you can then use the <Link href="/onClose">`onClose()` hook</Link> to clear resources.
...
export async function* onSearch(query: string) {
  ...
  onClose(() => cursor.close()) // client cancelled → release the cursor
...
const results = onSearch('telefunc')
cancelButton.onclick = () => results.return() // cancel → fires onClose() on the server
