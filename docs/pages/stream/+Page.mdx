...
*Telefunc Stream* enables streaming and real-time UIs via:
- Primitives
- Integrations
- Seamless DX
...
1. <Link href="#readable-stream"><code>ReadableStream</code></Link>:
   ...
   export async function onCompress(data: ReadableStream<Uint8Array>): Promise<ReadableStream<Uint8Array>> {
...
export async function onCompressVideo(video: ReadableStream<Uint8Array>): Promise<ReadableStream<Uint8Array>> {
  ...
  // A client that drops mid-upload rejects the pipe: stop the encoder rather than leave the rejection unhandled
  video.pipeTo(Writable.toWeb(ffmpeg.stdin)).catch(() => ffmpeg.kill())
