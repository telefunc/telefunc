...
> See:
> - <Link href="/onClose">`context.onClose()`</Link>
> - <Link href="/channel#new-channel">`channel.onClose()`</Link>

> The `channel.onClose()` hook can be used on the server- and client-side, whereas `context.onClose()` can only be used on the server.
