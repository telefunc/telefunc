...
*Telefunc Stream* supports streaming (one-way data flow) and real-time (two-way data flow) use cases via:
- Primitives
- Integrations
- Seamless DX

> With *stream*, we denote not only `ReadableStream` but, more broadly, any sequence of data made available over time. ([That's how Wikipedia defines a "stream"](https://en.wikipedia.org/wiki/Stream_(computing)).)
