...
> Integrations require even less code and handle more for you. They're powered by the primitives listed here.
...
>
> You can also use it for any other use case, such as never-ending streams — choose the primitive that gives you the nicest code. (While Telefunc picks a different default <Link href="/transport">transport</Link> depending on the primitive, you can change the transport to match your needs.)
...
> The underlying stream closes itself once the client stops referencing `onProgress`, but you can also close it eagerly yourself, see <Link href="/close" />.
...
You can also use a `signal` (`AbortSignal`) and `channel.onClose()` to detect when a stream closes, see:
- <Link href="/onClose" />
