...
> It's a good fit because a generator closes itself when it completes. (Other streaming primitives such <Link href="#callback">callbacks</Link> don't have such completion signal.)
...
> The stream automatically closes itself when the client stops using (i.e. referencing) `onProgress`, but you can also manually close eagerly, see <Link href="/close" />.

> Under the hood, a passed function *is* a <Link text={<code>Channel</code>} href="#channel" />. A callback has the same lifecycle as a channel, just exposed with a nice JavaScript DX.
>
...
> Like the other streaming primitives, you can use <Link href="/onClose">`onClose()`</Link> and <Link href="/close">`close`</Link> to eagerly cancel `fetchExtensiveReport()`
