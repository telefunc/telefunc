...
>
> You can also use it for any other use case, such as never-ending streams — the only deciding factor is DX. (Telefunc picks a different default <Link href="/transport">transport</Link> in that case, but you can change it.)
...
> The underlying stream closes itself automatically when the client stops referencing `onProgress`, but you can also close it eagerly yourself, see <Link href="/close" />.
