...
| <Link text={<code>download()</code>} href="/file-download#download" /> | File / Blob downloads with progress + cancel, or upstream-streamed bytes | Binary (`Uint8Array`) |
...
| <Link text={<code>new Channel().sendBinary()</code>} href="/channel#binary-data" /> | No | Fire-and-forget binary — low-latency frames where dropping is acceptable |
| <Link text={<code>new Broadcast().publishBinary()</code>} href="/channel#new-broadcast" /> | Publish-side only | Broadcast binary — video to multiple subscribers |
