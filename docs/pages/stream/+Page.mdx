...
> Here the word *stream* means — broadly speaking, [as Wikipedia defines it](https://en.wikipedia.org/wiki/Stream_(computing)) — not just a `ReadableStream` but any sequence of data made available over time.
...
> A single telefunction can [mix](https://gist.github.com/brillout/155c8eb4cbaa043bc52e96c0c2ed7086) generators, streams, and promises side by side, each resolving independently.
...
>
> You can also use it for any other use case, such as never-ending streams, if you want — the deciding factor is DX. (While Telefunc picks a different default <Link href="/transport">transport</Link> depending on the primitive, you can change the transport to match your needs.)
...
> The underlying stream automatically closes itself when the client stops using (i.e. referencing) `onProgress`, but you can also manually close it (eagerly), see <Link href="/close" />.
...
> As with other streaming primitives, you can use <Link href="/onClose">`onClose()`</Link> and <Link href="/close">`close`</Link> to eagerly cancel `fetchExtensiveReport()`.
...
Accept a `ReadableStream` *and* return one, transforming the bytes as they pass through. The simplest such transform is gzip compression:
...
A streaming telefunction authorizes like any other telefunction (see <Link href="/permissions" />): check <Link text="getContext()" href="/getContext" /> and `throw Abort()` **at open time**, before you stream anything back to the client. Here the client passes a <Link text="callback" href="#callback" /> and the server calls it:
...
You can scale Telefunc horizontally (across multiple instances, containers, or machines) by adding sticky sessions and a cross-instance broadcast transport.
