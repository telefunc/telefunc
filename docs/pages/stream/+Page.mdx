...
Telefunc supports streaming (one-way data flow) and real-time (two-way data flow) use cases with:
- Primitives
- Integrations
- Seamless DX
...
If the generator runs to completion, it closes automatically. For open-end generators, or when you stop early, close it from the client with `break`, `gen.return()`, or `close(gen)`.

If the server starts upstream work while yielding, tie it to close signals:
...
export async function* onAiPrompt(prompt: string): AsyncGenerator<string> {
  ...
  // Stop upstream work when the client disconnects or stops reading
...
A function-passing callback is channel-backed and can stay open as long as either side keeps a callback reference. For subscription-style usage, expose an explicit teardown function and also register the same teardown in `onClose()`:
...
export async function onSubscribe(onNotification: (text: string) => void) {
  const { onClose } = getContext()
  const unsubscribe = notifications.subscribe((n) => onNotification(n.text))
  onClose(unsubscribe) // server-side cleanup on disconnect/close
  return unsubscribe   // client-side explicit close
}
```
...
const stop = await onSubscribe((text) => showNotification(text))
...
Concurrent promises usually clean up automatically: once each promise settles, that branch is done. There is nothing to manually close if you simply `await` the returned promises.

If a promise wraps cancellable work, wire it to `getContext().signal` (or `onClose()`) so it aborts when the call closes.
...
**Client → server** — pass a `ReadableStream` as an argument to stream bytes to the server:
...
A `ReadableStream` closes automatically when it is fully consumed. If the client stops early, close it with `reader.cancel()`, `break` out of `for await`, or `close(stream)`.

On the server, release any external resource tied to the stream in `onClose()` (as shown with `ffmpeg.kill()` above), so disconnects do not leak work.
...
`Channel` / `BroadcastChannel` stay open until one side closes them. Close client-side channels when UI scope ends (`channel.close()` for graceful close, `channel.abort()` for immediate close), and register server-side teardown with `channel.onClose(...)` as shown above.


## Cleanup

Every streaming call eventually closes: normal completion, explicit client close, navigation/disconnect, or error. Cleanup hooks fire regardless of the closing reason.

To avoid leaks, always apply both rules:

1. **Server rule**: release any resource opened by the call (`setInterval`, event subscriptions, DB cursors, child processes, upstream streams) by wiring teardown to close signals (`getContext().onClose()`, `getContext().signal`, `channel.onClose()`).
2. **Client rule**: when UI no longer needs a live value, close it explicitly instead of dropping references.

Primitive-specific cleanup mechanisms are documented in each primitive section:

| Primitive | Cleanup details |
|---|---|
| `AsyncGenerator` | <Link href="#async-generator" /> |
| `function` passing | <Link href="#function-passing" /> |
| Concurrent `Promise` | <Link href="#concurrent-promise" /> |
| `ReadableStream` | <Link href="#readable-stream" /> |
| `Channel` / `BroadcastChannel` | <Link href="#channel" /> |

For nested return values, `close(value)` from `telefunc/client` gracefully closes all streams/channels/files recursively.

Exhaustive close/lifecycle API reference: <Link href="/onClose" />.
