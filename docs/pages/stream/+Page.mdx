...
✅ **Type validation at runtime** — every value the client sends to the server is shield-validated against your TypeScript types. See <Link href="/shield" />.  
...
✅ **Errors** — expected failures stay distinct from bugs, even mid-stream: if a stream throws, the client receives an error while values already delivered remain available. See <Link href="/error-handling#streaming" />.  
