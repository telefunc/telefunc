...
For example, write an `async function*` and read each value as it arrives:
...
For ongoing two-way and broadcast messaging, Telefunc has two primitives, plus a third that composes them:
...
> **Consider re-checking.** For sensitive operations, re-check authorization with the captured `user` each time the callback runs. Most use cases don't need this.
