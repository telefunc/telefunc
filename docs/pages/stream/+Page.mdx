...
> With *stream*, we denote not only `ReadableStream` but, more broadly, any sequence of data made available over time. ([That's how Wikipedia defines *streams*](https://en.wikipedia.org/wiki/Stream_(computing)).)
...
> This is the low-level primitive; most apps reach for a higher-level integration like <Link text={<code>@telefunc/tanstack-query</code>} href="/tanstack-query" /> instead.
