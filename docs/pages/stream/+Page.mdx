...
A short callback call closes with the telefunction call. Long-lived callback subscriptions should expose an explicit teardown function and also register the same teardown in `onClose()`:
