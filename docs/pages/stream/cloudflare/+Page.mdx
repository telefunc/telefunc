...
>
> The session Durable Object and KV bindings are required whenever you use `telefunc/cloudflare`. Rooms additionally require the SQLite `TelefuncRoomDurableObject` binding shown above. `new Telefunc()` selects that Room backend automatically; don't call `installRoomBackend()` for the standard Cloudflare setup. A missing Room binding fails with a setup diagnostic instead of falling back to per-isolate memory. If your app doesn't use `Channel`, `Broadcast`, or `Room`, there's no state to keep — you can skip these bindings and use <Link href="/serve"><code>serve()</code></Link> instead.
>
...
Retained Room payloads are chunked internally into rows of at most 1.5 MB and installed by an atomic manifest swap. The retained aggregate limit is 16 MiB per room generation; 3 MiB, 8 MiB, and 16 MiB payloads stay on this same backend path. The application still publishes and reads one payload — chunking isn't part of the Room API.
