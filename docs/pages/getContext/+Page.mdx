...
The context object also includes a `signal` (an `AbortSignal`) that aborts when the call ends. Pass it to APIs that accept an `AbortSignal` (`fetch`, database clients, etc.) so in-flight work cancels when the call ends.
