...
The context object includes `onClose()`, a callback that fires exactly once when the call ends — whether the client received everything, the client disconnected, or an error occurred. Use it to release resources.
...
The context object includes a `signal` (an `AbortSignal`) that aborts when the call ends. Pass it to APIs that accept an `AbortSignal` (`fetch`, database clients, etc.) so in-flight work cancels when the client disconnects.
