...
Cloudflare-specific setup for <Link href="/streaming">real-time</Link> (i.e. <Link href="/channel">`Channel` and `BroadcastChannel`</Link>).
...
**CDN / reverse proxy** — make sure `x-telefunc-session` is not stripped from responses or requests. Without it, each request would be routed to a random Durable Object and channel state would be lost.
...
Broadcasts (<Link href="/channel#new-broadcastchannel" />) fan out across all regions automatically.
