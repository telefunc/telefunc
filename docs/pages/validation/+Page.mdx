...
Telefunc guarantees argument types at runtime with a <Link href="/shield"><code>shield()</code></Link>. **This is generated automatically if you're using TypeScript.**

> If you aren't using TypeScript, or want to define the `shield` yourself, you can also <Link href="/shield#typescript-manual">define the <code>shield</code> manually</Link>.

The `shield` rejects telefunction calls with invalid argument types using <Link href="/Abort"><code>throw Abort()</code></Link>. 
...
export async function onFormSubmit(email: string, password: string) {
  // Form validation
  const inputErrors: { email?: string; password?: string } = {}
