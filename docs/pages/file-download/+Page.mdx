...
Return any number of `File`, `Blob`, or `download()` values anywhere in the response — in arrays, in nested objects, or mixed with regular data. Each `download()` exposes its own `onProgress` / `cancel` / `saveTo*` methods on the client, independently of the others, and each `File` or `Blob` arrives as a promise of it.
...
thumbEl.src = URL.createObjectURL(await thumbnail)
```
