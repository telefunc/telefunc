...
On the server, Telefunc cancels the streamed source automatically when the client cancels — aborting the S3 read for you, no cleanup needed:
...
export async function onGetImage(key: string) {
  // Client cancelled → Telefunc cancels this stream, aborting the S3 read
  const res = await s3.send(new GetObjectCommand({ Bucket: 'my-bucket', Key: key }))
  return download(res.Body!.transformToWebStream(), {
    name: key,
    size: res.ContentLength,
...
> Holding *extra* resources alongside the stream (a DB cursor, a temp file)? Release them with <Link text={<code>onClose()</code>} href="/onClose" />.
