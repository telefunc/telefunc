...
Stop a download early like any <Link text="stream" href="/stream#cancelling" /> — call `dl.cancel()` (or `reader.cancel()` on `dl.stream()`). The pending read / `saveTo*` then rejects with a "Stream cancelled" error.
...
> See <Link href="/stream#cancelling" /> for all the ways to stop a stream.


## Error handling

Same as any <Link text="stream" href="/stream#error-handling" />: the read rejects on failure (connection lost, source error, or cancel), and the expected-vs-bug distinction from <Link href="/error-handling" /> applies.
...
try {
  ...
  // Connection lost, upstream errored, or the download was cancelled
}
```

> See <Link href="/stream#error-handling" /> and <Link href="/error-handling" />.
