...
The streaming download behaves like a standard [`File`](https://developer.mozilla.org/en-US/docs/Web/API/File) / [`Blob`](https://developer.mozilla.org/en-US/docs/Web/API/Blob) — `dl.stream()`, `dl.text()`, `dl.arrayBuffer()`, `dl.bytes()`, and `dl.slice()` all work and pull from the streaming source as you read:

| Method | Returns | Memory | Best for |
|---|---|---|---|
| `dl.stream()` | `ReadableStream` | Constant (chunk-sized) | Pipe to disk, S3, or any writable stream |
| `dl.bytes()` | `Uint8Array` | Full file in memory | Process raw bytes |
