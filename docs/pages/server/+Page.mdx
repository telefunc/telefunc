...
Telefunc integrates with any server. For production deployments with streaming, channels, and WebSocket support, use the runtime-specific server entry — the `telefunc()` factory on Node.js, Bun, and Deno, or `new Telefunc()` on Cloudflare Workers.
