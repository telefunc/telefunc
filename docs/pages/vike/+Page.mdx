...
Install Telefunc's server middleware <Link href="/telefunc">`serve()`</Link>, for example if you use Hono:
...
<MostlyMutations
  ...
  builtInMechanism={
    <p>
      For example, you can use Vike's <code>+data()</code> hook to fetch initial data directly from a database,
      see{' '}
