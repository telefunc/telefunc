...
The client default is `['sse', 'ws']` — start on SSE, then upgrade to WebSocket once the server offers it. The server offers SSE out-of-the-box, and adds WebSocket if you set up your server via <Link href="/Telefunc">`new Telefunc()`</Link> (instead of <Link href="/serve">`serve()`</Link>).
