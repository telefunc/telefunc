...
The `@telefunc/rxjs` integration lets you pass RxJS `Observable` and `Subject` directly between client and server, in both directions and with all operators.
...
Unsubscribing stops data flow immediately. The underlying <Link href="/channel" text="channel" /> is cleaned up automatically via GC, or immediately with <Link href="/onClose" text="close()" />.
