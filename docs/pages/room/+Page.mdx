...
Guards ride the **grant**: `join(meta, { onSend })` guards that membership, and `Room.get(id, { onSend: (from, to, data) => ... })` guards every membership granted through the returned room instance — including client-side `join()`s on it (a membership's own `onSend` takes precedence). `Room.send()` (room-authored) is never guarded. Guards are declared server-side by construction — authorization code can't run on the machine it polices.
