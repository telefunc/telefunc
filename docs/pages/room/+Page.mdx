...
Your own handle, returned by `join()`. One type, same on server and client — return it from a telefunction as-is. Publish-only: receiving happens on the room and on `RemoteParticipant`.
