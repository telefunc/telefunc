...
| Method | Description |
|---|---|
| `join(meta?, options?)` | Join the room. Returns your own <Link href="#localparticipant">`LocalParticipant`</Link>. Options: `{ selfDelivery?, onSend? }`. |
...
- To authorize whispers, attach a guard to the join that grants membership: `join(meta, { onSend })` — see <Link href="#policy-gated-private-messages">the recipe</Link>.
...
To authorize whispers (e.g. friends-only DMs), attach an `onSend` guard to the join that grants membership. It's defined next to your authentication — closing over the request context — and runs before every delivery; throwing rejects the sender's `send()` with the error:
...
export async function onJoinChat(roomId: string) {
  const { user } = getContext()
  return await Room.join(roomId, { name: user.name, userId: user.id }, {
    onSend: async (to) => {
      if (!(await db.areFriends(user.id, to.meta.userId))) throw new Error('not friends')
    },
  })
}
```
...
The guard travels with the membership: sends from this participant — from the server or from the client holding it — pass through it, while `Room.send()` (room-authored) doesn't. Since the guard requires a server-side join, hand out participants (not the room) when memberships must be policed.
