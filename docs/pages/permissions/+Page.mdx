...
To implement reusable permissions logic, define helper functions that call <Link href="/getContext"><code>getContext()</code></Link>.
...
export async function onTextChange(todoId, text) {
  ...
  const todo = await db.query(
    'SELECT id, text, authorId FROM todos WHERE id = :todoId LIMIT 1',
    { todoId }
  )
  ...
  if (todo.authorId !== user.id && !user.isAdmin) {
...
> You can use the same `getContext()` and `throw Abort()` pattern <Link href="/stream#authorization">to protect streams</Link>.

Use `throw Abort()` when rejecting access to a resource, or if a telefunction call is otherwise invalid.
...
/**
 * @param {string} text
 * @returns {Promise<OnNewTodoResult>}
 */
