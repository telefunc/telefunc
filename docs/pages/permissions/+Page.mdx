...
> The same `getContext()` + `throw Abort()` pattern protects <Link href="/stream">streaming and real-time</Link> telefunctions — channels and broadcasts authorize at open time. See <Link href="/stream#authorization" />.
