...
Combine file upload with <Link href="/stream#primitive-function-passing">function passing</Link> to report progress while the server reads the stream.
...
File bytes flow directly from the HTTP stream to your code with zero internal buffering. This makes uploads memory-efficient, with two trade-offs:
