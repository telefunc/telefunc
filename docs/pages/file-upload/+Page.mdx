...
You can cancel a file upload at anytime with <Link href="/close">`close()`</Link> (client-side) — you can then use the <Link href="/onClose">`onClose()` hook</Link> (server-side) to clear resources — see <Link href="/stream#cleanup" />.
