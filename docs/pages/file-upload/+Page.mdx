...
You can pass [`File`](https://developer.mozilla.org/en-US/docs/Web/API/File) and [`Blob`](https://developer.mozilla.org/en-US/docs/Web/API/Blob) arguments to a telefunction like any other value. A single file, multiple file arguments, a `File[]` argument, or files mixed with other arguments — all are supported.
