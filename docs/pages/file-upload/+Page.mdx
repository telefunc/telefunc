...
To save resources, you can cancel a file upload at anytime by <Link href="/close">manually closing it</Link> — you can then use the <Link href="/onClose">`onClose()` hook</Link> to clear resources — see <Link href="/stream#cleanup" />.
...
const call = onUpload(file, onProgress)
cancelButton.onclick = () => close(call) // stops the upload mid-stream
```

On the server, use the <Link text={<code>onClose()</code>} href="/onClose" /> hook to release resources when the upload is cancelled:
...
export async function onUpload(file: File, onProgress: (percent: number) => void) {
  ...
  // Upload cancelled → close the file and discard the partial write
  onClose(() => {
    dest.destroy()
    fs.rm(`./uploads/${file.name}`, { force: true }, () => {})
  })
...
> **You don't always need `onClose()`.** A cancelled upload surfaces as a thrown error on the next `file.stream()` read, so you only need to clean up if you opened a resource that won't release itself (a write stream, a temp file, a DB transaction). When that's the case, a plain `try`/`finally` around the read loop works too:
>
> ```ts
> try {
>   for await (const chunk of file.stream()) dest.write(chunk)
>   dest.end()
> } finally {
>   if (!dest.closed) dest.destroy() // cancelled mid-read → release the resource
> }
> ```
>
> And if you `pipeTo` a destination (`file.stream().pipeTo(dest)`), it's aborted for you when the upload is cancelled — no cleanup needed.


## Error handling

You handle errors the same way as you do for Telefunc streams: <Link href="/stream#error-handling" />.
