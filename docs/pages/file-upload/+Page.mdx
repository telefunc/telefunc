...
An upload is a regular telefunction call, so you cancel it like any <Link text="stream" href="/stream#cancelling" />: `abort()` the in-flight call from `telefunc/client`.
...
cancelButton.onclick = () => abort(call) // stops the upload mid-stream
await call
```
...
Same as any <Link text="stream" href="/stream#error-handling" />: the call rejects on failure, and the expected-vs-bug distinction from <Link href="/error-handling" /> applies — `throw Abort()` is relayed as an expected error, any other thrown error is treated as a bug.
...
try {
  ...
  // Connection lost, server threw, or the call was aborted
}
```
