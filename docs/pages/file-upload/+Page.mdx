...
cancelButton.onclick = () => close(call) // gracefully cancels the upload
```
...
> **Cancelling is an expected path, not an error** — Telefunc fires `onClose()` and never treats the client going away as a bug. You only need `onClose()` to release a resource that won't free itself (a write stream, a temp file, a DB transaction). If you `pipeTo` a destination (`file.stream().pipeTo(dest)`), it's aborted for you when the upload is cancelled — no cleanup needed.
