...
The client-side `onAbort()` hook is called whenever telefunction calls fail because of <Link href="/Abort"><code>throw Abort()</code></Link>.
...
onAbort((error) => {
  const { code } = error.abortValue as { code: string }
